...
p1 = run(['vlog', 'ref_no_f.sv'])
p2 = run(['vlog', 'only_f.sv'])
...
with open("ref.vcd", "rb") as f:
    ...
    for i, token in enumerate(tokens):
        if token.kind == vcd.reader.TokenKind.VAR and token.data.reference == 'f_t':
...
for model_type in ['llm', 'gpt3.5', 'gpt4', 'claude', 'llama', 'mistral', 'gemini']:
    for file_name in glob.glob(f"completions_{model_type}/*.v"):
        ...
        if p.returncode == 0:
            ...
            p2_1 = run(['vlog', 'ref_no_f.sv'])
            p2_2 = run(['vlog', 'testbench.sv'])
            if p2_1.returncode != 0 or p2_2.returncode != 0:
                print("error, can't compile testbench or ref_no_f")
                break
            ...
            if p3.returncode != 0:
                ...
                with open("dump.vcd", "rb") as f:
                    ...
                    try:
                        ...
                        d.append([file_name, model_type, prompt_detail, temp, completion_id, 1, 1, editdistance.eval(f_track, f_ref), get_area(file_name, "f", "NA")])
                    except vcd.reader.VCDParseError as e:
                        d.append([file_name, model_type, prompt_detail, temp, completion_id, 1, 1, 'NA', 'NA'])
        else:
