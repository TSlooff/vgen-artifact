...
p1 = run(['vlog', 'ref_no_h.sv'])
p2 = run(['vlog', 'only_h.sv'])
...
with open("ref.vcd", "rb") as f:
    ...
    for i, token in enumerate(tokens):
        if token.kind == vcd.reader.TokenKind.VAR and token.data.reference == 'h_t':
...
for model_type in ['llm', 'gpt3.5', 'gpt4', 'claude', 'llama', 'mistral', 'gemini']:
    for file_name in glob.glob(f"completions_{model_type}/*.v"):
        ...
        if p.returncode == 0:
            ...
            p2_1 = run(['vlog', 'ref_no_h.sv'])
            p2_2 = run(['vlog', 'testbench.sv'])
            if p2_1.returncode != 0 or p2_2.returncode != 0:
                print("error, can't compile testbench or ref_no_h")
                break
            try:
            ...
            if p3.returncode != 0:
                ...
                try:
                    with open("dump.vcd", "rb") as f:
                        ...
                        d.append([file_name, model_type, prompt_detail, temp, completion_id, 1, 1, editdistance.eval(f_track, f_ref), get_area(file_name, 'h', 'NA')])
