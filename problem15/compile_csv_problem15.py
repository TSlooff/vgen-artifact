...
d.append(["REF.txt", "manual", "NA", "NA", "NA", 1, 1, 1, ref_CTs[0], 0, get_area('finalround.v', 'Finalround', 'NA')])
        ...
        if p.returncode == 0:
            ...
            if p3.returncode != 0:
                ...
                with open("CT.txt", "r") as f:
                    CTs = f.readlines()
                d.append([file_name, model_type, prompt_detail, temp, completion_id, 1, 1, int(CTs == ref_CTs), CTs[0] if len(CTs) != 0 else 'NA', sum(editdistance.eval(a, b) for (a,b) in zip(CTs, ref_CTs)), get_area(file_name, 'Finalround', 'NA')])
        else:
