...
p1 = run(['vlog', 'ref_no_keyschedule.v'])
p2 = run(['vlog', 'keyschedule.v'])
p3 = run(['vcom', 'present_tb.vhd'])
p4 = run("""timeout --preserve-status 5m vsim -c present_tb -do 'run 100us;quit'""", shell=True)
...
d.append(["REF.txt", "manual", "NA", "NA", "NA", 1, 1, 1, ref_CTs[0], 0, get_area('keyschedule.v', 'keyschedule', 'NA')])
...
for model_type in ['llm', 'gpt3.5', 'gpt4', 'claude', 'llama', 'mistral', 'gemini']:
    for file_name in glob.glob(f"completions_{model_type}/*.v"):
        ...
        if p.returncode == 0:
            ...
            p2_1 = run(['vlog', 'ref_no_keyschedule.v'])
            p2_2 = run(['vcom', 'present_tb.vhd'])
            if p2_1.returncode != 0 or p2_2.returncode != 0:
                print("error, can't compile testbench or quark_functions")
                break
            p3 = run("""timeout --preserve-status 5m vsim -c present_tb -do 'run 100us;quit'""", shell=True)
            if p3.returncode != 0:
                # can not run bench
                ...
                with open("CT.txt", "r") as f:
                    CTs = f.readlines()
                d.append([file_name, model_type, prompt_detail, temp, completion_id, 1, 1, int(CTs == ref_CTs), CTs[0] if len(CTs) != 0 else 'NA', sum(editdistance.eval(a, b) for (a,b) in zip(CTs, ref_CTs)), get_area(file_name, 'keyschedule', 'NA')])
        else:
