...
p1 = run(['vlog', 'ref_no_g.sv'])
p2 = run(['vlog', 'only_g.sv'])
...
with open("ref.vcd", "rb") as f:
    ...
    for i, token in enumerate(tokens):
        if token.kind == vcd.reader.TokenKind.VAR and token.data.reference == 'g_t':
...
# ['file_name', 'model_type', 'prompt_detail', 'temp', 'completion_id', 'compiled', 'passed_bench', 'Levenshtein distance']
for model_type in ['claude', 'llm', 'gpt3.5', 'gpt4', 'llama', 'mistral', 'gemini']:
    for file_name in glob.glob(f"completions_{model_type}/*.v"):
        ...
        if p.returncode == 0:
            ...
            p2_1 = run(['vlog', 'ref_no_g.sv'])
            p2_2 = run(['vlog', 'testbench.sv'])
            if p2_1.returncode != 0 or p2_2.returncode != 0:
                print("error, can't compile testbench or ref_no_g")
                break
            try:
                with open("p3.log", "a") as f:
                    f.write(f"{file_name}\n")
                p3 = run("""vsim -c -novopt -warning vsim-12110 quark_tb -do 'run 11000; quit'""", shell=True, timeout=30)
            ...
            if p3.returncode != 0:
                ...
                try:
                    with open("dump.vcd", "rb") as f:
                        ...
                        d.append([file_name, model_type, prompt_detail, temp, completion_id, 1, 1, editdistance.eval(f_track, f_ref), get_area(file_name, "g", "NA")])
