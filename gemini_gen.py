    ...
    for prompt_detail in ['L', 'M', 'H']:
        ...
        for t in [0.1, 0.3, 0.5, 0.7, 1.0]:
            ...
            for i in tqdm(range(0,100), desc=f"generating completions for problem={problem_id}, prompt detail={prompt_detail}, t={t}", leave=False):
                if not os.path.exists(f"problem{problem_id}/completions_gemini/completion_{prompt_detail}_t{t}_{i}.v"):
                    while True:
                        try:
                            ...
                            with open(f"problem{problem_id}/completions_gemini/completion_{prompt_detail}_t{t}_{i}.v", "w") as f:
