...
for problem_id in problem_ids:
    os.makedirs(f"problem{problem_id}/completions_gpt3.5/", exist_ok=True)
    for prompt_detail in ['L', 'M', 'H']:
        ...
        for t in [0.1, 0.3, 0.5, 0.7, 1.0]:
            ...
            for i in tqdm(range(0,100,n)):
                if not os.path.exists(f"problem{problem_id}/completions_gpt3.5/completion_{prompt_detail}_t{t}_{i}.v"):
                    chat_completion = client.chat.completions.create(
                        messages=[
                            ...
                            {
                                "role": "user",
                                "content": prompt + "\nGive the full implementation of this function",
                            }
                        ],
                        seed = seeds[i],
                        temperature=t,
                    ...
                    for j in range(n):
                        with open(f"problem{problem_id}/completions_gpt3.5/completion_{prompt_detail}_t{t}_{i+j}.v", "w") as f:
