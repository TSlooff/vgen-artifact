...
d.append(["REF.txt", "manual", "NA", "NA", "NA", 1, 1, 1, ref_CTs[0], 0, get_area('mixcolumn.v', 'Mixcolumn', 'NA')])
for model_type in ['gemini', 'llm', 'gpt3.5', 'gpt4', 'claude', 'llama', 'mistral']:
    for file_name in glob.glob(f"completions_{model_type}/*.v"):
        ...
        if p.returncode == 0:
            ...
            if p2_1.returncode != 0 or p2_2.returncode != 0:
                ...
                break
            if "completion_H_t0.3_49.v" in file_name:
                d.append([file_name, model_type, prompt_detail, temp, completion_id, 1, 0, 0, 'NA', 'NA', 'NA'])
                continue
            try:
                p3 = run("""timeout --preserve-status 5m vsim -c aes_tb -do 'run 100us;quit'""", shell=True, timeout=600)
            ...
            if p3.returncode != 0:
                ...
                with open("CT.txt", "r") as f:
                    CTs = f.readlines()
                d.append([file_name, model_type, prompt_detail, temp, completion_id, 1, 1, int(CTs == ref_CTs), CTs[0] if len(CTs) != 0 else 'NA', sum(editdistance.eval(a, b) for (a,b) in zip(CTs, ref_CTs)), get_area(file_name, 'Mixcolumn', 'NA')])
        else:
